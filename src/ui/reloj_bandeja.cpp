...
// El dibujo comun: la pastilla de color y un texto centrado, al tamaño que se
// pida. Lo comparten el reloj y la cuenta atras.
QIcon pastillaConTexto(const QColor& fondo, const QString& arriba, const QString& abajo) {
    QIcon icono;
    for (int lado : {22, 24, 32, 48, 64}) {
        ...
        fuente.setBold(true);
        const bool una_linea = abajo.isEmpty();
        int px = una_linea ? lado : lado / 2;
        for (; px > 5; --px) {
            fuente.setPixelSize(px);
            const QFontMetrics fm(fuente);
            const bool cabe_ancho = fm.horizontalAdvance(arriba) <= lado - 2 &&
                                    fm.horizontalAdvance(abajo) <= lado - 2;
            const bool cabe_alto = !una_linea || fm.height() <= lado;
            if (cabe_ancho && cabe_alto) break;
        }
        fuente.setPixelSize(px);
        p.setFont(fuente);
        p.setPen(Qt::white);
        if (una_linea) {
            p.drawText(QRect(0, 0, lado, lado), Qt::AlignCenter, arriba);
        } else {
            p.drawText(QRect(0, 0, lado, lado / 2), Qt::AlignCenter, arriba);
            p.drawText(QRect(0, lado / 2, lado, lado - lado / 2), Qt::AlignCenter, abajo);
        }
        p.end();
